        ...
        try:
            ...
            while True:
                ...
                if len(data) == 0:
                    ...
                    break
                elif data == b'QUERY_IMAGE':
                    result, imgencode = cv2.imencode('.jpg', g_frame, encode_param)
                    data = np.array(imgencode).tobytes()
                    header = struct.pack('HHHHII', len(data), g_proximity[0],  g_proximity[1], g_proximity[2],
                                          g_wheel_count[0], g_wheel_count[1])
                    connection.sendall(header + data)
                    print('\t\tsending data to the client. (%d bytes)' % len(data))
                else:
                    print('\t\tunexpected command.')

        except socket.error as e:
            print('\tsocket.error: %s' % e.args[0])
